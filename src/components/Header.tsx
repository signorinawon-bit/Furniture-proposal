...
import { Sofa, CheckCircle2, SlidersHorizontal, Layers, FileCheck, Share2 } from 'lucide-react';
    ...
    <header className="bg-white border-b border-slate-200 sticky top-0 z-30 shadow-xs print:hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3.5">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          ...
          <div className="flex items-center justify-between">
            ...
            {onOpenOgModal && (
              <button
                ...
                className="md:hidden p-2 rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-100 text-xs font-semibold transition-colors cursor-pointer"
                title="배포용 오픈 그래프 미리보기"
                aria-label="오픈 그래프 미리보기"
              >
          ...
          {/* Stepper Steps & OG Preview Button */}
          <div className="flex items-center justify-between md:justify-end gap-3">
            ...
            {onOpenOgModal && (
              <button
                type="button"
                onClick={onOpenOgModal}
                className="hidden md:flex items-center gap-1.5 px-3 py-2 rounded-xl border border-slate-200 text-slate-700 hover:bg-slate-100 hover:border-slate-300 text-xs font-semibold transition-all cursor-pointer shrink-0 ml-2"
                title="배포 시 SNS 링크 미리보기 (Open Graph)"
              >
                <Share2 className="w-3.5 h-3.5 text-slate-500" />
                <span>배포 OG 미리보기</span>
              </button>
            )}
          </div>
        </div>
